...
class MasterData extends CI_Controller
{
    ...
    {
        $this->db->trans_start($id);

        // cek apakah data ditabel terkait masih ada yang menggunakan nilai ID ini
    ...
    {
        $this->form_validation->set_rules('username', 'Username', 'required|trim|min_length[8]|matches[password2]');
        $this->form_validation->set_rules('nama', 'Nama', 'required|trim||matches[password1]');
    ...
    public function petugas_pass()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->session->set_flashdata('success',  '<div class"alert alert-success" role="alert">Data ditambah</div>');
                redirect('masterdata/petugas');
            } else {
                return false;
            }
    ...
    public function add_siswa()
    {
        ...
        $this->form_validation->set_rules('NISN', 'NISN', 'required|trim|is_unique[tbl_siswa.NISN]|max_length[10]|min_length[10]', [
    ...
    public function siswa_del($id)
    {
        ...
        // cek apakah data ditabel terkait masih ada yang menggunakan nilai ID ini
        $cek = $this->db->get_where('tbl_pembayaran', ['nisn' => $id, 'tgl_bayar !=' => null]);
        ...
        if ($this->db->trans_status() === false) {
            ...
            $this->session->set_flashdata(
                'message',
                '<div class="alert alert-danger" role"alert">Data siswa berhasil didelete</div>'
            );
            redirect('masterdata/siswa');
            //transaksi berhasil, tampilkan pesan sukses
        }
    ...
    public function jurusan_del($id)
    {
        ...
        // cek apakah data ditabel terkait masih ada yang menggunakan nilai ID ini
        $cek = $this->db->get_where('tbl_kelas', ['id_jurusan' => $id]);
        if ($cek->num_rows() > 0) {
            // Ada data terkait,rollback transaksi dan hentikan proses penghapusan
            $this->db->trans_rollback();
            $this->session->set_flashdata(
        ...
        if ($this->db->trans_status() === false) {
            ...
            $this->session->set_flashdata(
                'message',
                'Data jurusan berhasil didelete'
            );
            redirect('masterdata/jurusan');
            //transaksi berhasil, tampilkan pesan sukses
        }
