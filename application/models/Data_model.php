...
class Data_model extends CI_Model
{
    ...
    {
        $query = "SELECT `tbl_petugas`.*, `tbl_level`.* FROM `tbl_petugas`, `tbl_level` WHERE `tbl_petugas`.`id_level` = `tbl_level`.`id_level`";
    ...
    {
        $query = "SELECT `tbl_pembayaran`.*, `tbl_siswa`.* , `tbl_spp`.`TAHUN`, `tbl_spp`.`NOMINAL`, `tbl_kelas`.`nama_kelas`, `tbl_jurusan`.`jurusan`
        FROM `tbl_pembayaran`,`tbl_siswa`,`tbl_spp`,`tbl_kelas`,`tbl_jurusan` WHERE `tbl_pembayaran`.`nisn` = `tbl_siswa`.`nisn` AND `tbl_siswa`.`id_kelas` = `tbl_kelas`.`id_kelas` AND `tbl_siswa`.`id_spp` = `tbl_spp`.`id_spp` AND `tbl_kelas`.`id_jurusan` = `tbl_jurusan`.`id_jurusan` AND `tbl_pembayaran`.`KET` = 'LUNAS' AND `tbl_siswa`.`NISN` LIKE '%" . $keyword . "%' 
